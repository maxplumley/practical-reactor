...
public class c5_CreatingSequence {
    ...
    public void generate_programmatically() {
        ...
        Flux<Integer> generateFlux = Flux.generate(sink -> {
            var value = generateCount.getAndIncrement();
            if (value <= 5) {
                sink.next(value);
            } else {
                sink.complete(); //todo: fix following code so it emits values from 0 to 5 and then completes
            }
        });
        ...
        var pushCount = new AtomicInteger(0);
        Flux<Integer> pushFlux = Flux.push(sink -> {
            while (pushCount.get() <= 5) {
                sink.next(pushCount.getAndIncrement());
            }
            sink.complete(); //todo: fix following code so it emits values from 0 to 5 and then completes
        });
