...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void mail_box_switcher() {
        ...
        Flux<Message> myMail = mailBoxPrimary()
                .take(1)
                .filter(mail -> !mail.metaData.equals("spam"))
                .switchIfEmpty(mailBoxSecondary());

        // or use switchOnFirst()
    ...
    public void acid_durability() {
        //todo: feel free to change code as you need
        Flux<String> committedTasksIds = tasksToExecute().flatMapSequential(task -> task.flatMap(id ->
            commitTask(id).thenReturn(id)
        ));
    ...
    public Mono<String> chooseSource() {
        return Mono.defer(() -> {
            switch (sourceRef.get()) {
                case "A": return sourceA();
                case "B": return sourceB();
